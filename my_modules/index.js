...
function add_nav_item(text) {
    var nav_div = document.getElementById("nav_div");
    if (nav_div == null)
        return;
    var nav_item_div = document.createElement("div");
    nav_item_div.className = "nav_item_div";
    nav_item_div.innerHTML = text;
    nav_div.appendChild(nav_item_div);
}
var all_title_div = [];
export function add_text_title_item(text, nav_name, html_link) {
    var title_div = document.getElementById("title_div");
    if (title_div == null)
        return;
    var title_item_div = document.createElement("div");
    ...
    title_item_div.addEventListener('click', function () {
        if (nav_name != "") {
            collapse_menu.change_title_selection(nav_name);
        }
        all_title_div.forEach(function (e) {
